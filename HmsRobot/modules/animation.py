...
hack_you = [
    "البحث عن قواعد بيانات WhatsApp في الشخص المستهدف ...",
...
def hack(update: Update, context: CallbackContext):
    bot, args = context.bot, context.args
    msg = update.effective_message.reply_text("الهدف المحدد")
    for x in range(EDIT_TIMES):
        msg.edit_text(hack_you[x % 5])
        time.sleep(EDIT_SLEEP)
    msg.edit_text("تم اختراق جميع البيانات المرسلة في قاعدة البيانات الخاصة بي بنجاح")
