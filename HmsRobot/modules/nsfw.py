...
from HmsRobot import dispatcher, updater
import HmsRobot.modules.sql.nsfw_sql as sql
...
def add_nsfw(update: Update, context: CallbackContext):
    ...
    if not is_nsfw:
        ...
        msg.reply_text("تم تنشيط وضع NSFW بالفعل لهذه الدردشة!")
        return ""
...
def rem_nsfw(update: Update, context: CallbackContext):
    ...
    is_nsfw = sql.is_nsfw(chat.id)
    if not is_nsfw:
        msg.reply_text("وضع NSFW غير نشط بالفعل")
        return ""
    else:
        sql.rem_nsfw(chat.id)
        msg.reply_text("التراجع إلى وضع SFW!")
        message = (
            f"<b>{html.escape(chat.title)}:</b>\n"
            f"DEACTIVATED_NSFW\n"
            f"<b>Admin:</b> {mention_html(user.id, html.escape(user.first_name))}\n"
...
def keta(update, context):
    ...
    target = 'keta'
    if not target:
        msg.reply_text("لم يتم استلام عنوان URL من API!")
        return
    msg.reply_photo(nekos.img(target))
